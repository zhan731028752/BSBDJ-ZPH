...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //打开网页
    //1.UIApplication ,跳出应用程序,
    //2.UIWebView ,功能单一.只是打开一般的网页.没有地址栏,没有进度条
    //3.SFSafariViewController,ios9.0,
    //webKit :是UIWebView升级版,有地址栏,而且可以监听到加载进度,前进,后退,刷新
    if([item.url hasPrefix:@"http"]){
//        [self.navigationController pushViewController:safariViewController animated:YES];
        self.navigationController.navigationBar.hidden=YES;
        //下面该方法可以代理-(void)safariViewControllerDidFinish:(SFSafariViewController *)controller和 [self.navigationController pushViewController:safariViewController animated:YES];
